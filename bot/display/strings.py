...
class AllStrings(Enum):
    ...
    REG_ALREADY_OWN = Message(
        "You are already registered with the following Jaeger characters: `{}`, `{}`, `{}`"
    )
    REG_ALREADY_NOA = Message(
        "You are already registered without a Jaeger account! If you have your own "
    ...
    REG_NOT_JAEGER = Message(
        "Invalid registration! Character `{}` doesn't belong to Jaeger!"
    )
    REG_ALREADY_EXIST = Message(
        "Invalid registration! Character `{}` is already registered by {}!"
    )
    REG_MISSING_FACTION = Message(
        "Invalid registration! Can't find a {} character in your list!"
    )
    REG_UPDATE_OWN = Message(
        "You successfully updated your profile with the following Jaeger characters:"
        " `{}`, `{}`, `{}`"
    )
    REG_UPDATE_NOA = Message(
        "You successfully removed your Jaeger characters from your profile."
    )
    REG_WITH_CHARS = Message(
        "You successfully registered with the following Jaeger characters: `{}`, `{}`, `{}`"
    )
    REG_FROZEN = Message("You can't register while you're playing a match!")
    REG_RULES = Message(
        "{} You have accepted the rules, you may now register.",
    ...
    LB_STUCK = Message(
        "Lobby is full, but can't start a new match yet. Please wait...", ping=False
    )
    LB_STUCK_JOIN = Message("You can't join the lobby, it is already full!")
    LB_MATCH_STARTING = Message(
        "Lobby full, match can start! Join <#{}> for team selection!",
        ping=False,
        embed=embeds.join_ts,
    )
    LB_WARNING = Message(
        "{} you will be timed out of the lobby soon! Use `=reset` to remain in the queue!"
    )
    LB_TOO_LONG = Message(
        "{} was removed from the lobby by timeout!", embed=embeds.lobby_list
    )
    ...
    LB_REFRESHED = Message(
        "You have reset your queue timeout!", embed=embeds.lobby_list
    )
    ...
    PK_NO_LOBBIED = Message(
        "You must first queue and wait for a match to begin. Check <#{}>"
    )
    PK_WAIT_FOR_PICK = Message(
        "You can't do that! Wait for a Team Captain to pick you!"
    )
    ...
    PK_PLAYERS_HELP = Message(
        "Waiting for {} to pick a player with `=p @mention`", ping=False
    )
    ...
    PK_OK = Message(
        "Player picked! {} your turn, pick a player!",
        embed=embeds.team_update,
        ping=False,
    )
    PK_P_OK = Message(
        "Picked {}! {} your turn, pick a player!", embed=embeds.team_update, ping=False
    )
    ...
    PK_OK_FACTION = Message(
        "Teams are ready! {} pick a faction with `=pick` `tr`/`vs`/`nc`!", ping=False
    )
    ...
    PK_FACTION_OK_NEXT = Message(
        "{} chose {}! {} pick a faction with `=pick` `tr`/`vs`/`nc`!", ping=False
    )
    PK_FACTION_CHANGED = Message("{} changed to {}!", ping=False)
    PK_FACTION_NOT_PLAYER = Message("Pick a faction, not a player!")
    PK_FACTION_INFO = Message(
        "Captains can change faction with `=pick` `tr`/`vs`/`nc`!"
    )
    PK_FACTION_HELP = Message(
        "Waiting for {} to pick a faction with `=pick` `tr`/`vs`/`nc`!", ping=False
    )
    PK_BASING_INFO = Message(
        "Waiting for captains to pick a base with `=base`!\n"
        "Captains can change faction with `=pick` `tr`/`vs`/`nc`!"
    )

    EXT_NOT_REGISTERED = Message("You are not registered! Check <#{}>")
    UNKNOWN_ERROR = Message(
        "Something unexpected happened! Please try again or contact staff if it keeps happening.\n"
        "Details: *{}*"
    )
    ...
    INVALID_COMMAND = Message(
        "Invalid command! Type `=help` for the list of available commands."
    )
    ...
    API_READY_ERROR = Message(
        "Could not reach Planetside2 API, player online check ignored!", ping=False
    )
    API_SCORE_ERROR = Message(
        "Match {}, round {}: Could not reach Planetside2 API, no scores for this round!"
    )
    PUBLISH_ERROR = Message(
        "Match {}, round {}: Could not publish score image, no scores for this round!"
    )
    GLOBAL_INFO = Message(
        "Here is what's going on in POG at the moment:", embed=embeds.global_info
    )
    CHECK_ACCOUNT = Message(
        "Your account password may have been flipped!\n"
        "Re-register in <#{}> to confirm you still have access to it!",
        embed=embeds.flip_accounts,
    )
    ...
    BOT_DM_RECEIVED = Message(
        "Thanks for your message, it was forwarded to POG staff!", ping=False
    )
    ...
    CAP_WAITING = Message(
        "Waiting for captain(s), volunteer if you want to be one",
        ping=False,
        embed=embeds.team_update,
    )
    ...
    CAP_AUTO = Message(
        "{} has been designated as captain for {}\n" "Accept if you agree!"
    )
    CAP_HELP = Message(
        "Here are the available captain commands:", embed=embeds.captain_help
    )
    ...
    MATCH_DM_PING = Message(
        "POG match {} is starting! Please join `{}` channel in the Jaeger Events discord!\n{}",
        ping=False,
    )
    MATCH_INIT = Message("{}\nMatch is ready, starting team selection...")
    MATCH_SHOW_PICKS = Message(
        "Captains have been selected, {} choose a player",
        embed=embeds.team_update,
        ping=False,
    )
    MATCH_BASE_AUTO = Message("Match will be on **{}**", ping=False)
    MATCH_CONFIRM = Message(
        "{} {} Type `=ready` when your team is inside their sunderer, ready to start",
        embed=embeds.team_update,
    )
    ...
    MATCH_STARTING_1 = Message(
        "Everyone is ready, round {} is starting in {} seconds!\nAll players will be pinged on "
        "round start"
    )
    ...
    MATCH_PLAYERS_NOT_READY = Message(
        "Can't get {} ready, {} did not accept their Jaeger accounts", ping=False
    )
    MATCH_GETTING_READY = Message(
        "{} getting ready: checking for players...", ping=False
    )
    MATCH_PLAYERS_OFFLINE = Message(
        "Can't get {} ready, {} {} not online in game!",
        ping=False,
    ...
    BASE_SELECTED = Message(
        "This match will be played on **{}**:", embed=embeds.base_display
    )
    BASE_DISPLAY = Message("Base navigator:", ping=False, embed=embeds.base_display)
    BASE_BOOKED = Message(
        "{} WARNING: **{}** seems unavailable. Please check occupation "
        "before confirming this base.",
        ping=False,
        embed=embeds.jaeger_calendar,
    )
    # BASE_ADDED = Message("Added {} to the base pool")
    # BASE_REMOVED = Message("Removed {} from the base pool")
    BASE_CALENDAR = Message(
        "{} Pick an available base!", ping=False, embed=embeds.jaeger_calendar
    )
    ...
    ACC_NOT_ENOUGH = Message(
        "Not enough accounts are available for this match!\n**Match has been canceled!**"
    )
    ACC_ERROR = Message(
        "Error when giving out Jaeger accounts!\n**Match has been canceled!**"
    )
    ACC_UPDATE = Message(None, ping=False, embed=embeds.account)
    ACC_STAFF = Message(
        "{}, couldn't send the account to {}, please send it manually...",
        ping=False,
    ...
    ACC_OVER = Message(
        "Match is over, please log out of your Jaeger account!", ping=False
    )
    ACC_CLOSED = Message(
        "{}'s DMS are locked, couldn't send them a Jaeger account after 3 retries!\nSending the "
    ...
    ACCOUNT_USAGE = Message(
        "Here is the POG account usage for this user:", embed=embeds.usage
    )
    DISPLAY_USAGE = Message(
        "<@{}> played {} POG match{} in the last {}. \n(since {})", ping=False
    )
    PSB_USAGE = Message(
        "Here is the participation for {}, for 8 weeks leading up to {}:",
        ping=False,
        embed=embeds.psb_usage,
    )
    ...
    NOT_CODED = Message(
        "The rest is not yet coded, work in progress. Clearing match..."
    )
    ...
    RM_TIMEOUT_INVALID = Message(
        "Invalid use of the command!", embed=embeds.timeout_help
    )
    ...
    SC_ILLEGAL_WE = Message(
        "{} used {} during match {}! This weapon is banned! Ignoring {} kill(s)..."
    )
    SC_PLAYERS_STRING = Message("Here is player data for squittal script:\n{}\n")
    SC_PLAYERS_STRING_DISC = Message(
        "Here is player data for squittal script:\n{}\n"
    ...
    SUB_NO_PLAYER = Message(
        "Subbing {}: no player is available to substitute!", ping=False
    )
    SUB_OKAY_TEAM = Message(
        "{} replaced {} in {}", ping=False, embed=embeds.team_update
    )
    SUB_OKAY_CAP = Message(
        "{} replaced {} as {}'s captain", ping=False, embed=embeds.team_update
    )
    SUB_OKAY = Message("{} replaced {}!", ping=False, embed=embeds.team_update)
    SUB_LOBBY = Message(
        "{} you have been designated as a substitute, join <#{}>!",
        embed=embeds.lobby_list,
    )
    ...
    SWAP_OK = Message(
        "Successfully swapped {} and {}", ping=False, embed=embeds.team_update
    )
    SWAP_MENTION_2 = Message("Invalid request! @ mention two players to swap!")
    SWAP_NO = Message("{} can't be swapped!", ping=False)
    SWAP_SAME_TEAM = Message(
        "Invalid request! Can't swap two players of the same team!", ping=False
    )
    ...
    UNBENCH_OK = Message(
        "Successfully un-benched {}!", ping=False, embed=embeds.team_update
    )
    BENCH_ALREADY = Message("Player is already benched!", ping=False)
    BENCH_NOT = Message("Player is not benched!", ping=False)
    BENCH_ALL = Message(
        "Can't bench {}, no active player left in the team!", ping=False
    )
