...
def register_help(ctx):
    ...
    embed.add_field(
        name="If you don't have a Jaeger account",
        value="`=r no account`\n",
        inline=False,
    )
    embed.add_field(
        name="If you have a Jaeger account",
...
def captain_help(ctx):
    ...
    embed = Embed(colour=Color.blurple())
    embed.add_field(
        name="Captain selection commands",
...
def register_status(ctx, player):
    ...
    if player.has_own_account:
        ...
        value = (
            "You are registered without a Jaeger account!\nIf you have your own "
...
def account(ctx, account):
    ...
    if account.is_destroyed:
        ...
        desc = (
            "Accept the rules by reacting with a checkmark to get your account details."
        )
        color = Color.red()
    embed = Embed(
        colour=color,
...
def lobby_list(ctx, names_in_lobby):
    ...
    embed.add_field(
        name=f'Lobby: {len(names_in_lobby)} / {cfg.general["lobby_size"]}',
        value=list_of_names,
        inline=False,
    )
    return embed
...
def offline_list(ctx, p_list):
    ...
    embed.add_field(
        name=f"The following {'players are' if len(p_list) > 1 else 'player is'} not online ingame:",
...
def team_update(ctx, match):
    ...
    for tm in match.teams:
        if tm.captain:
            ...
            if (
                match.next_status in (MatchStatus.IS_FACTION, MatchStatus.IS_PICKING)
                and tm.captain.is_turn
            ):
            ...
            if tm.faction != 0:
                faction = (
                    f"{cfg.emojis[cfg.factions[tm.faction]]} {cfg.factions[tm.faction]}"
                )
    ...
    if name:
        players_string = "\n".join(
            [f"- {p.mention} ({p.name})" for p in match.get_left_players()]
        )
...
def jaeger_calendar(arg):
    ...
    date = dt.now(tz.utc)
    embed.add_field(
        name="Current UTC time", value=date.strftime("%Y-%m-%d %H:%M UTC"), inline=False
    )
    return embed
...
def base_display(ctx, base, is_booked):
    ...
    if base.id in cfg.base_images:
        ...
        embed.add_field(
            name=f"Image", value="Not available for this base", inline=False
        )
    return embed
...
def direct_message(ctx, player, msg):
    ...
    if player:
        ...
        description += (
            f" from unregistered user {msg.author.mention} (id: {msg.author.id})\n"
        )
    embed = Embed(
        colour=Color.dark_grey(), title="Direct Message", description=description
    )
    embed.add_field(name=f"Message:", value=msg.content, inline=False)
    return embed
...
def usage(ctx, data):
    ...
    embed = Embed(
        colour=Color.blue(), title="Last account usages", description=description
    )
    if is_account:
        embed.add_field(
            name="Account used by:",
...
def psb_usage(ctx, player, usages):
    ...
    for use in usages:
        ...
        week = (
            f"{use.week_num} week{'s' if use.week_num > 1 else ''} ago"
            if use.week_num
            else f"Current week"
        )
